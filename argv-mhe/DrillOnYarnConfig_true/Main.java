...
public class Main {
 ...
 public String getRemoteDrillHome() throws Exception {
    ...
    if (home.equals(BASE_NAME_MARKER)) {
      ...
      drillHome += "/" + home.substring(0, home.length() - suffix.length()) + "z";
    } else {
      drillHome += "/" + home + "z";
    }
    assert drillHome.matches("a.*/.*z"); //inline assert generated by ARG-V - should also mean always has a parent if not starting with slash
